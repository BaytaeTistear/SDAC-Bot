...
from dashboard_admin_roles import (
...
from dashboard_shell_assets import PWA_HEAD_HTML, PWA_INSTALL_HTML, SIDEBAR_STYLE
from dashboard_sidebar import (
    admin_sidebar_html as build_admin_sidebar_html,
    admin_sidebar_links as build_admin_sidebar_links,
    admin_sidebar_sections as build_admin_sidebar_sections,
    public_sidebar_sections as build_public_sidebar_sections,
    should_render_admin_sidebar as build_should_render_admin_sidebar,
    should_render_public_sidebar as build_should_render_public_sidebar,
    sidebar_sections as build_sidebar_sections,
)
...
<main>
    ...
    <section class="panel">
        <h2>Achievement Totals</h2>
        <table>
            <thead><tr><th>Achievement</th><th>Awards</th></tr></thead>
            <tbody>
                {% for row in achievement_totals %}
                    <tr><td>{{ row.label }}</td><td>{{ row.award_count }}</td></tr>
                {% else %}
                    <tr><td colspan="2" class="muted">No achievements have been awarded for this view yet.</td></tr>
                {% endfor %}
            </tbody>
        </table>
    </section>
    <section class="panel">
        ...
        <table>
            <thead><tr><th>User</th><th>Achievement</th><th>Details</th><th>Awarded</th></tr></thead>
            <tbody>
                {% for row in achievement_rows %}
                    <tr>
                        <td>{{ row.username }}</td>
                        <td>{{ row.label }}{% if row.month %}<br><code>{{ row.month }}</code>{% endif %}</td>
                        <td>{{ row.details or "" }}</td>
                        <td>{{ row.awarded_at }}</td>
                    </tr>
                {% else %}
                    <tr><td colspan="4" class="muted">No recent achievements for this view yet.</td></tr>
                {% endfor %}
            </tbody>
        </table>
    </section>
    ...
    <section class="panel">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="grid">
                ...
                <label>Pack
                    <input name="pack_name" maxlength="80" placeholder="Movie night, music pack, season 1">
                </label>
                <label>Tags
                    <input name="tags" maxlength="400" placeholder="easy, 2000s, comedy">
                </label>
            ...
            <label>
                ...
                Enabled for scheduled/random library games
            </label>
            ...
            <label>Admin notes
                <textarea name="notes" maxlength="1000" placeholder="Optional private notes for admins"></textarea>
            </label>
                ...
                {% for item in items %}
                    <tr>
                        ...
                        <td>
                            ...
                            {% if item.pack_name %}Pack: {{ item.pack_name }}<br>{% endif %}
                            {% if item.tags %}Tags: {{ item.tags|join(', ') }}<br>{% endif %}
                            {% if item.prompt_text %}<span class="muted">{{ item.prompt_text }}</span>{% endif %}
                            {% if item.notes %}<br><span class="muted">Notes: {{ item.notes }}</span>{% endif %}
                        </td>
                        <td>
                            {% if item.media_url %}
                        ...
                        <td>
                            <code>{{ item.status }}</code><br>
                            <span class="{{ '' if item.enabled else 'muted' }}">{{ "Enabled" if item.enabled else "Picker disabled" }}</span>
                        </td>
                        <td>
                        ...
                        <td class="actions">
                            <details>
                                ...
                                <form method="post" enctype="multipart/form-data">
                                    ...
                                    <input name="pack_name" value="{{ item.pack_name or '' }}" placeholder="Pack">
                                    <input name="tags" value="{{ item.tags|join(', ') }}" placeholder="Tags">
                                    ...
                                    <textarea name="notes" placeholder="Admin notes">{{ item.notes or '' }}</textarea>
                                    <select name="status">
                                    ...
                                    <label>
                                        <input type="hidden" name="enabled" value="0">
                                        <input name="enabled" type="checkbox" value="1" {% if item.enabled %}checked{% endif %}>
                                        Enabled for scheduled/random games
                                    </label>
            ...
            <form method="post">
                ...
                <table>
                    <tbody>
                        ...
                        <tr><th>Backup zip archives</th><td>
                            <select name="external_backup_zip_backups">
                                <option value="1" {% if guild.external_backup.zip_backups %}selected{% endif %}>Enabled</option>
                                <option value="0" {% if not guild.external_backup.zip_backups %}selected{% endif %}>Disabled</option>
                            </select>
                        </td></tr>
            ...
            <table>
                <tbody>
                    ...
                    <tr><th>External backup</th><td>
                        ...
                        <div>Zip archives: <code>{{ "Yes" if guild.external_backup.zip_backups else "No" }}</code></div>
...
<main>
    ...
    <section class="panel">
        ...
        <p>Rollback to a known release:</p>
        <p><code>sudo sdac-update 3.0.14</code></p>
...
<main>
    ...
    <section class="grid">
        <article class="card">
            <h2>Service Logs</h2>
            <p>Last bot logs:</p>
            <p><code>sudo journalctl -u sdac-bot -n 80 --no-pager</code></p>
            <p>Follow bot logs live:</p>
            <p><code>sudo journalctl -u sdac-bot -f</code></p>
            <p>Service status:</p>
        ...
        {% for row in servers %}
            <article class="card">
                ...
                <p>
                    Backup:
                    <span class="{{ 'ok' if row.backup.enabled and row.backup.remote else 'bad' }}">
                        {{ "configured" if row.backup.enabled and row.backup.remote else "needs setup" }}
                    </span><br>
                    Remote: <code>{{ row.backup.remote or "Not set" }}</code><br>
                    Last archive: <code>{{ row.last_archive.status or "None" }}</code>
                    {% if row.last_archive.created_at %} at {{ row.last_archive.created_at }}{% endif %}
                </p>
                <p class="muted">
                    Useful Discord commands:
                    <code>/setupchecklist</code>,
                    <code>/backupstatus</code>,
                    <code>/scheduledgames</code>
                </p>
            </article>
        {% else %}
            <p class="muted">No servers are available to this dashboard admin.</p>
        ...
        for option in guild_options(config_data):
            ...
            stats = {
                ...
                "library_items": connection.execute("""
                    ...
                    WHERE guild_id = ?
                """, (guild_id,)).fetchone()[0],
                "enabled_library_items": connection.execute("""
                    SELECT COUNT(*)
                    FROM guess_library_items
                    WHERE guild_id = ?
                      AND status = 'active'
            ...
            rows.append({
                ...
                "name": option["name"],
                "setup_score": (
                    onboarding.get(guild_id, {}).get("health_score", 0)
                ),
                "storage": forecasts.get(guild_id),
                "backup": backup,
...
def parse_metadata_list(raw_value, limit=24, item_limit=40):
    values = []
    seen = set()
...
def parse_enabled_field(raw_value, default=True):
    value = str(raw_value if raw_value is not None else "").strip().casefold()
    if not value:
        return 1 if default else 0
    if value in {"1", "true", "yes", "y", "on", "enabled", "active"}:
        return 1
...
def build_onboarding_rows(config_data):
    ...
    for guild_id, guild_config in sorted(
        ...
        items = [
            ...
            onboarding_item(
                backup.get("enabled") and backup.get("remote"),
                "Per-server backup target",
                "/backupguide provider then /backupsetup",
                optional=True,
            ),
            onboarding_item(
                backup.get("zip_backups", True),
                "Zip backup archives enabled",
                "/backupsetup zip_backups:true",
                optional=True,
            ),
            onboarding_item(
                digest.get("enabled") and digest.get("channel_id"),
...
def is_bot_owner_username(username):
    username = str(username or "").strip().casefold()
    if not username:
        return False
    if username == OWNER_OVERRIDE_USERNAME:
        return True
    try:
        with closing(connect_db()) as connection:
            row = connection.execute("""
                SELECT 1
                FROM dashboard_bot_owners
...
def current_requested_guild_id():
    if not has_request_context():
        return ""
    requested = request.values.get("guild_id", "").strip()
    if requested == "all":
        return ""
    return requested or session.get("sdac_guild_id", "")
...
def dashboard_user_server_access(username=""):
    ...
    if not raw_username:
        return []
    username = normalize_account_username(raw_username)
    with closing(connect_db()) as connection:
        return connection.execute("""
            SELECT username, guild_id, role, source, verified_at, updated_at
...
def dashboard_user_server_access_map(username=""):
    return {
        str(row["guild_id"]): normalize_role(row["role"])
        for row in dashboard_user_server_access(username)
    }


def upsert_user_server_access(connection, username, guild_ids, role="user", source="manual", preserve_existing_roles=True):
    username = normalize_account_username(username)
    role = normalize_role(role)
    now = utc_now_iso()
    rows = []
    for guild_id in sorted({str(item).strip() for item in guild_ids if str(item).strip()}):
        existing = connection.execute("""
            SELECT role
            FROM dashboard_user_server_access
            WHERE username = ? AND guild_id = ?
        """, (username, guild_id)).fetchone()
        row_role = normalize_role(existing["role"]) if existing and preserve_existing_roles else role
        connection.execute("""
...
def dashboard_user_max_role(username, fallback_role="user"):
    username = normalize_account_username(username)
    if is_bot_owner_username(username):
        return "bot_owner"
    max_role = normalize_role(fallback_role)
    for row in dashboard_user_server_access(username):
        row_role = normalize_role(row["role"])
        if ROLE_LEVELS[row_role] > ROLE_LEVELS[max_role]:
            max_role = row_role
    return max_role
...
def public_sidebar_sections():
    return build_public_sidebar_sections()
...
def should_render_public_sidebar():
    return build_should_render_public_sidebar()
...
def admin_sidebar_html():
    return build_admin_sidebar_html(
        ...
        is_account_logged_in=is_account_logged_in,
        current_admin_role=current_admin_role,
        current_admin_username=current_admin_username,
        is_bot_owner_username=is_bot_owner_username,
        current_account_username=current_account_username,
        normalize_role=normalize_role,
        load_config=load_config,
        sidebar_server_options=sidebar_server_options,
        admin_url=admin_url,
    )
    ...
    if request.method == "POST":
        ...
        if login_rate_limited(remote_key):
            ...
            if user and not int(user["disabled"] or 0):
                ...
                role = "bot_owner" if is_bot_owner_username(user["username"]) else normalize_role(user["role"])
                role = dashboard_user_max_role(user["username"], role)
            ...
            if (
                ...
                with database() as connection:
                    upsert_user_server_access(
                        connection,
                        account["username"],
                        parse_guild_scope(account["guild_ids_json"]),
                        role=account_role,
                        source="legacy",
                    )
                    connection.execute("""
...
def account_oauth_callback():
    ...
    with database() as connection:
        ...
        role = "bot_owner" if is_bot_owner_username(account["username"]) else normalize_role(account["role"])
        scope_ids = sorted((set(config_data.get("guilds") or {}) if role == "bot_owner" else set(oauth_guild_ids)))
        upsert_user_server_access(
            connection,
            account["username"],
            scope_ids,
            role=role if role == "bot_owner" else "user",
            source="oauth",
        )
        ...
        if guild_id not in existing_scope:
            ...
            with database() as connection:
                upsert_user_server_access(
                    ...
                    [guild_id],
                    role=normalize_role(session.get("sdac_account_role")),
                    source="selected",
                )
                connection.execute("""
                    UPDATE dashboard_admin_users
...
@app.route("/account/discord/refresh")
def account_refresh_discord():
    if not is_account_logged_in():
        return redirect(url_for("account_login", next=request.full_path))
    return redirect(url_for(
        "account_oauth_start",
        next=request.args.get("next") or url_for("account_access_debug"),
    ))


@app.route("/account/access")
def account_access_debug():
    if not is_account_logged_in():
        return redirect(url_for("account_login", next=request.full_path))
    config_data = load_config()
    account = dashboard_user(current_account_username())
    access_map = dashboard_user_server_access_map(current_account_username())
    allowed_ids = current_account_allowed_guild_ids(config_data)
    admin_ids = current_admin_allowed_guild_ids(config_data) if is_admin_logged_in() else set()
    rows = []
    for guild_id, guild_config in sorted(
        (config_data.get("guilds") or {}).items(),
        key=lambda item: (item[1].get("guild_name") or item[0]).casefold(),
        ...
        guild_id = str(guild_id)
        role = access_map.get(guild_id, normalize_role(account["role"] if account else "user"))
        if is_bot_owner_username(current_account_username()):
            role = "bot_owner"
        reason = "per-server access row" if guild_id in access_map else "legacy account scope"
        if guild_id not in allowed_ids:
            reason = "not in stored Discord/server access scope"
        rows.append({
            "guild_id": guild_id,
            "name": guild_config.get("brand_name") or guild_config.get("guild_name") or f"Discord {guild_id}",
            "account_visible": guild_id in allowed_ids,
            "admin_visible": guild_id in admin_ids,
            "role": role,
            "source": reason,
            "public_gallery": feature_enabled(guild_config, "public_gallery"),
            "cross_server": feature_enabled(guild_config, "cross_server_gallery"),
        })
    return render_template_string("""
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        h1 { text-align: center; }
        a { color: #22d3ee; }
        .panel { background: #111827; border: 1px solid #334155; border-radius: 8px; margin: 16px 0; padding: 16px; }
        table { border-collapse: collapse; width: 100%; }
        th, td { border-bottom: 1px solid #334155; padding: 10px; text-align: left; vertical-align: top; }
        code { color: #c7d2fe; }
        .muted { color: #94a3b8; }
        .ok { color: #86efac; font-weight: 800; }
        .no { color: #fca5a5; font-weight: 800; }
        .actions { display: flex; flex-wrap: wrap; gap: 10px; justify-content: center; }
        .actions a { background: linear-gradient(90deg, #4f46e5, #06b6d4); border-radius: 7px; color: white; font-weight: 800; padding: 10px 12px; text-decoration: none; }
        @media (max-width: 760px) {
            body { padding: 12px; }
            table, thead, tbody, tr, th, td { display: block; }
            thead { display: none; }
            tr { border-bottom: 1px solid #334155; padding: 10px 0; }
            th, td { border-bottom: 0; padding: 6px 0; }
        }
    </style>
</head>
...
<main>
    <h1>Access Debug</h1>
    <div class="actions">
        <a href="{{ url_for('account_refresh_discord', next=request.full_path) }}">Refresh Discord Servers</a>
        <a href="{{ url_for('account_server', next=request.full_path) }}">Change Server</a>
        <a href="{{ url_for('account_home') }}">My Account</a>
    </div>
    <section class="panel">
        <h2>Current Session</h2>
        <p>Username: <code>{{ current_account_username }}</code></p>
        <p>Account role: <code>{{ account_role }}</code></p>
        <p>Admin role on selected server: <code>{{ admin_role }}</code></p>
        <p>Discord user id: <code>{{ discord_user_id or '' }}</code></p>
        <p>Selected server: <code>{{ selected_guild_id or 'all' }}</code></p>
        {% if not rows %}
            <p class="no">No configured servers were found.</p>
        {% elif not allowed_ids %}
            <p class="no">No servers are linked to this account. Use Refresh Discord Servers or ask a Bot Owner to assign access.</p>
        {% endif %}
    </section>
    <section class="panel">
        <h2>Servers</h2>
        <table>
            <thead><tr><th>Server</th><th>Visible</th><th>Admin Visible</th><th>Your Role</th><th>Source</th><th>Features</th></tr></thead>
            <tbody>
            {% for row in rows %}
                <tr>
                    <td>{{ row.name }}<br><code>{{ row.guild_id }}</code></td>
                    <td><span class="{{ 'ok' if row.account_visible else 'no' }}">{{ 'Yes' if row.account_visible else 'No' }}</span></td>
                    <td><span class="{{ 'ok' if row.admin_visible else 'no' }}">{{ 'Yes' if row.admin_visible else 'No' }}</span></td>
                    <td>{{ role_labels.get(row.role, row.role) }}</td>
                    <td>{{ row.source }}</td>
                    <td>
                        Public gallery: {{ 'on' if row.public_gallery else 'off' }}<br>
                        Cross-server: {{ 'on' if row.cross_server else 'off' }}
                    </td>
                </tr>
            {% else %}
...
""",
        account_role=normalize_role(account["role"] if account else session.get("sdac_account_role")),
        admin_role=current_admin_role() if is_admin_logged_in() else "not-admin",
        allowed_ids=allowed_ids,
        current_account_username=current_account_username(),
        discord_user_id=session.get("sdac_discord_user_id", ""),
        role_labels=ROLE_LABELS,
        rows=rows,
        selected_guild_id=current_requested_guild_id(),
    )
    ...
    if request.method == "POST":
        ...
        try:
            if action == "create_item":
                ...
                pack_name = request.form.get("pack_name", "").strip()[:80]
                tags = parse_metadata_list(request.form.get("tags", ""))
                notes = request.form.get("notes", "").strip()[:1000]
                enabled = parse_enabled_field(request.form.get("enabled"), True)
                try:
                    ...
                    if new_role == "bot_owner":
                        connection.execute("""
                            ...
                            ON CONFLICT(username) DO UPDATE SET
                                ...
                                updated_at = excluded.updated_at
                        """, (username, now, now))
                    elif username != OWNER_OVERRIDE_USERNAME:
                        connection.execute("""
                            DELETE FROM dashboard_bot_owners
                            WHERE lower(username) = ?
                        """, (username,))
...
@app.route("/admin/preview-as")
def admin_preview_as():
    ...
    username = normalize_account_username(
        request.args.get("username") or (users[0]["username"] if users else current_admin_username())
    )
    guild_options_rows = guild_options(config_data)
    guild_id = request.args.get("guild_id", "").strip()
    if not guild_id and guild_options_rows:
        guild_id = str(guild_options_rows[0]["id"])
    user = dashboard_user(username)
    access_map = dashboard_user_server_access_map(username)
    role = "bot_owner" if is_bot_owner_username(username) else normalize_role(user["role"] if user else "user")
    if guild_id and guild_id in access_map:
        role = access_map[guild_id]
    allowed_ids = set(config_data.get("guilds") or {}) if is_bot_owner_username(username) else set(access_map)
    if not allowed_ids and user:
        allowed_ids = set(parse_guild_scope(user["guild_ids_json"]))
    visible_sections = []
    for label, required_role in [
        ("User", "user"),
        ...
        visible_sections.append({
            "label": label,
            "visible": ROLE_LEVELS[role] >= ROLE_LEVELS[required_role],
            "required": ROLE_LABELS[required_role],
        })
    return render_template_string("""
<!DOCTYPE html>
<html lang="en">
...
<main>
        ...
        <form method="get">
            ...
            <label>User
                <select name="username">
                    {% for row in users %}
                        <option value="{{ row.username }}" {% if row.username == username %}selected{% endif %}>{{ row.username }}</option>
                    {% endfor %}
                </select>
            </label>
            <label>Server
                <select name="guild_id">
                    {% for guild in guilds %}
                        <option value="{{ guild.id }}" {% if guild.id == guild_id %}selected{% endif %}>{{ guild.name }} ({{ guild.id }})</option>
                    {% endfor %}
                </select>
            </label>
    ...
    <section class="panel">
        <h2>Result</h2>
        <p>User: <code>{{ username }}</code></p>
        <p>Server: <code>{{ guild_id or 'all' }}</code></p>
        <p>Effective role: <strong>{{ role_labels.get(role, role) }}</strong></p>
        <p>Server access: <span class="{{ 'ok' if guild_id in allowed_ids else 'bad' }}">{{ 'Allowed' if guild_id in allowed_ids else 'Not linked' }}</span></p>
        <p class="muted">This is a read-only preview. It does not change your session or perform actions as the selected user.</p>
    </section>
    <section class="panel">
        <h2>Sidebar Sections</h2>
        <table>
            <thead><tr><th>Section</th><th>Required</th><th>Visible</th></tr></thead>
            <tbody>
                {% for section in visible_sections %}
                    <tr>
                        <td>{{ section.label }}</td>
                        <td>{{ section.required }}</td>
                        <td><span class="{{ 'ok' if section.visible else 'bad' }}">{{ 'Yes' if section.visible else 'No' }}</span></td>
                    </tr>
                {% endfor %}
            </tbody>
        </table>
                ...
                if action == "update_guild":
                    ...
                    guild_config["external_backup"] = {
                        ...
                        **existing_backup,
                        ...
                        "zip_backups": (
                            request.form.get("external_backup_zip_backups") == "1"
                        ),
...
def admin_server_health_cards():
    ...
    return render_template_string(
        SERVER_HEALTH_CARDS_HTML,
        admin_key=ADMIN_KEY,
        servers=server_health_card_rows(),
    )
...
def achievements():
    ...
    with closing(connect_db()) as connection:
        ...
        """, submission_params).fetchone()
        achievement_where = [
            "substr(COALESCE(awarded_at, ''), 1, 7) = ?",
        ]
        achievement_params = [month]
        if selected_server_id:
            achievement_where.append("guild_id = ?")
            achievement_params.append(selected_server_id)
        else:
            achievement_filter, achievement_filter_params = guild_id_filter(
                "guild_id",
                visible_guild_ids,
            )
            achievement_where.append(achievement_filter)
            achievement_params.extend(achievement_filter_params)
        achievement_rows = connection.execute(f"""
            SELECT guild_id, username, achievement_key, label,
                   details, month, awarded_at
            FROM user_achievements
            WHERE {" AND ".join(achievement_where)}
            ORDER BY awarded_at DESC, id DESC
            LIMIT 25
        """, achievement_params).fetchall()
        achievement_totals = connection.execute(f"""
            SELECT label, COUNT(*) AS award_count
            FROM user_achievements
            WHERE {" AND ".join(achievement_where)}
            GROUP BY achievement_key, label
            ORDER BY award_count DESC, label ASC
        """, achievement_params).fetchall()
