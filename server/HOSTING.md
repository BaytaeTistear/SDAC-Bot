...
On a fresh Linux server, sign in as the account that should run Sana-Chan and paste this single command:
...
The command downloads the full installer to a temporary file, verifies the published SHA-256 checksum, installs system and Python dependencies, creates the environment file, installs systemd services, and starts the bot and dashboard. Interactive prompts collect the Discord token and first full-access account password without putting either secret in shell history.
