...
`/sdac` is always the fallback control center. Server owners can also set an optional server-specific launcher such as `/pepo` from `/sdac` -> Setup -> Command Name, or from the setup wizard's final page. They can change the bot's server nickname from `/sdac` -> Setup -> Bot Name; leaving the name blank resets it to the bot's global username. The bot needs Manage Nicknames and a high enough role to apply the nickname in Discord. Run `/sdac` -> Setup -> Sync Commands after changing the alias or if Discord still shows old duplicate commands. Discord may take a minute to show or remove commands after sync.
