...
- Capacitor is the fastest path to an official iOS and Android app because Sana-Chan already has a working responsive dashboard and PWA routes.
...
```bash
cd apps/sana-official-app
npm install
copy .env.example .env
npm run dev
```

The default dashboard URL is `https://sanachan.bot.nu`. For local dashboard testing, change `.env` to `http://127.0.0.1:5000`.

Set `VITE_SANA_DASHBOARD_URL` to the hosted dashboard URL for the Vite shell. Native builds always use the packaged Sana-Chan shell; the old direct-dashboard WebView wrapper mode has been removed.

The app name is configurable:

```powershell
$env:SANA_APP_NAME="Sana-Chan"
$env:SANA_APP_ID="com.baytae.sanachan"
```
...
```bash
SANA_APP_ALLOWED_ORIGINS=http://localhost:5174,capacitor://localhost
```
...
npm install
npm run build
$env:VITE_SANA_DASHBOARD_URL="https://sanachan.bot.nu"
$env:SANA_APP_NAME="Sana-Chan"
...
```powershell
cd apps\sana-official-app\android
...
```text
apps/sana-official-app/android/app/build/outputs/apk/debug/
```
...
```text
apps/sana-official-app/android/app/build/outputs/bundle/release/
```
...
keytool -genkeypair `
  -v `
  -keystore "$env:USERPROFILE\.sana-chan\android-signing\Sana-Chan-release.jks" `
  -alias Sana-Chan `
  -keyalg RSA `
  -keysize 2048 `
  -validity 10000
...
Create `apps/sana-official-app/android/keystore.properties` and do not commit it:

```properties
storeFile=C:\\Users\\YOUR_USER\\.sana-chan\\android-signing\\Sana-Chan-release.jks
storePassword=YOUR_STORE_PASSWORD
keyAlias=Sana-Chan
keyPassword=YOUR_KEY_PASSWORD
```

Then add signing config to `apps/sana-official-app/android/app/build.gradle`:
...
npm install
npm run build
export VITE_SANA_DASHBOARD_URL="https://sanachan.bot.nu"
export SANA_APP_NAME="Sana-Chan"
npm run cap:add:ios
npm run cap:sync
...
```text
apps/sana-official-app/public/sana-companion-art.png
```

It is an original anime companion illustration holding an community-style emblem. It intentionally avoids copying an official sports logo.
