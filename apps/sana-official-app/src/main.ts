...
const APP_SHELL_VERSION = "4.4.23";
...
function routeButton(label: string, route: string, extraClass = ""): string {
  ...
  if (route?.startsWith("#")) {
    return `<button class="${escapeHtml(className)}" type="button" data-app-action="scroll-to" data-target="${escapeHtml(route)}"${disabled}>${escapeHtml(label)}</button>`;
  }
  return `<button class="${escapeHtml(className)}" type="button" data-app-action="frame-route" data-route="${escapeHtml(route)}"${disabled}>${escapeHtml(label)}</button>`;
}
...
function setFrameRoute(route: string): void {
  const frame = document.querySelector<HTMLIFrameElement>("iframe");
  if (frame) {
    frame.src = appFrameUrl(route || "/");
  }
}
...
function bottomNav(payload: BootstrapPayload): string {
  ...
  const accountControl = loggedIn
    ? `<button type="button" data-app-action="frame-route" data-route="${escapeHtml(routes.account || "/account")}">Account</button>`
  ...
  return `
    <nav class="bottom-nav" aria-label="Primary app navigation">
      <button type="button" data-app-action="frame-route" data-route="${escapeHtml(routes.home || "/")}">Home</button>
      <button type="button" data-app-action="frame-route" data-route="${escapeHtml(routes.submissions || "/")}">Submit</button>
      <button type="button" data-app-action="frame-route" data-route="${escapeHtml(routes.guessing || "/guessing")}">Games</button>
      ${accountControl}
...
function diagnosticsPanel(payload: BootstrapPayload): string {
  ...
  return `
    <details class="panel diagnostics">
      ...
      <dl>
        <dt>Dashboard</dt><dd>${escapeHtml(payload.diagnostics.dashboard_url || dashboardBase)}</dd>
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-app-action]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        } else if (action === "frame-route") {
          setFrameRoute(button.dataset.route || "/");
...
function render(payload: BootstrapPayload): void {
  ...
  document.querySelector<HTMLSelectElement>("#server-select")?.addEventListener("change", (event) => {
    ...
    setFrameRoute(`${routes.submissions || "/"}?guild_id=${encodeURIComponent(guildId)}`);
  });
